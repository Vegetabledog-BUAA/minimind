...
class PretrainDataset(Dataset):
    def __init__(self, data_path_lst, max_length=512, memmap=False):
        ...
        if memmap:
            ...
            data = data[:max_length * int(len(data) / max_length)]  # 截取数据到 max_length 的整数倍
    ...
    def __getitem__(self, index: int):
        ...
        for history_message in history:
            ...
            if len(history_message) <= 1:
                continue
            # 由于历史对话有用户和助手两个角色，因此需要分别添加，且需要截取到最大长度的一半，从而保证总长度不超过最大长度
            messages.append(
                {"role": 'user', "content": str(history_message[0])[:self.max_length // 2]}  # 添加用户的历史消息
            )
            messages.append(
                {"role": 'assistant', "content": str(history_message[1])[:self.max_length // 2]} # 添加助手的历史消息
            )
